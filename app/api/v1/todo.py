...
def create_todo(todo: TodoCreate, db: Session = Depends(get_db)):
    ...
    return success_response(data=TodoOut.model_validate(result).model_dump(), message="Created", status_code=201)

@router.get("/todos")
def get_all(db: Session = Depends(get_db)):
    todos = todo_service.get_all_todos_service(db)
    return success_response(data=[TodoOut.model_validate(t).model_dump() for t in todos])
...
def get_by_id(id: int, db: Session = Depends(get_db)):
    ...
    return success_response(data=TodoOut.model_validate(todo).model_dump())
...
def update(id: int, data: TodoCreate, db: Session = Depends(get_db)):
    ...
    return success_response(data=TodoOut.model_validate(result).model_dump(), message="Updated")
