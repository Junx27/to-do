...
def create_status_todo(status_todo: StatusTodoCreate, db: Session = Depends(get_db)):
    ...
    return success_response(data=StatusTodoOut.model_validate(result).model_dump(), message="OK", status_code=201)
...
def get_all_status_todo(db: Session = Depends(get_db)):
    ...
    return success_response(data=[StatusTodoOut.model_validate(t).model_dump() for t in status_todos])
...
def get_status_todo_by_id(id: int, db: Session = Depends(get_db)):
    ...
    return success_response(data=StatusTodoOut.model_validate(status_todo).model_dump())
...
def update_status_todo(id: int, data: StatusTodoCreate, db: Session = Depends(get_db)):
    ...
    if not result:
        return success_response(message="Not found", status_code=404)
    return success_response(data=StatusTodoOut.model_validate(result).model_dump(), message="OK")
